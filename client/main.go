...
// -----END CERTIFICATE-----`
const awsRootCert = `https://www.amazontrust.com/repository/AmazonRootCA1.pem`
...
const cvm = `https://6bhfrjpxo2.execute-api.us-east-1.amazonaws.com/LATEST/getcert?serialNumber=%v&deviceToken=%v`
...
func main() {
	...
	resp, err := http.Get(awsRootCert)
	if err != nil {
		log.Fatal(err)
	}
	defer resp.Body.Close()
	awsPEM, err := ioutil.ReadAll(resp.Body)

	roots := x509.NewCertPool()
	ok := roots.AppendCertsFromPEM(awsPEM)
	if !ok {
	...
	creds := cred.NewStaticCredentials(a.Credentials.AccessKeyID, a.Credentials.SecretAccessKey, a.Credentials.SessionToken)
	...
	ht, _ := httputil.DumpRequestOut(req, false)
